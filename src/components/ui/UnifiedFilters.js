...
const UnifiedFilters = ({
  ...
  const filterSummary = useMemo(() => getFilterSummary(), [filters, getFilterSummary]);
  ...
  const handleMunicipalityToggle = (municipality) => {
    ...
    // Trigger debounced refresh after municipality change
  ...
  const handleRegionToggle = (regionKey) => {
    if (onToggleRegion) {
      onToggleRegion(regionKey);
    }

    // Trigger debounced refresh after region change
    debouncedRefresh(300);
  };
  ...
  return (
    <div className={cn("bg-white/80 backdrop-blur-md border border-white/30 rounded-2xl shadow-lg relative", className)}>
      <div className="p-6 border-b border-slate-200">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center space-x-4 flex-1 min-w-0 relative">
                    ...
                    {DATE_PRESETS.map((preset) => (
                      <button
                        ...
                        onClick={() => {
                          ...
                          setShowDatePresets(false);
                          // Trigger debounced refresh for date preset changes
                    ...
                    <div className="border-t border-slate-200 pt-3 mt-3">
                      <div className="space-y-2">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            onUpdateFilter('startDate', new Date(e.target.value));
                            // Trigger debounced refresh for custom dates
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            onUpdateFilter('endDate', new Date(e.target.value));
                            // Trigger debounced refresh for custom dates
                            debouncedRefresh(800);
                          }}
                          className="w-full h-8 px-2 text-sm bg-white border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
            ...
            <select
              ...
              onChange={(e) => {
                onUpdateFilter('technician', e.target.value);
                debouncedRefresh(100);
              }}
            ...
            <select
              ...
              onChange={(e) => {
                onUpdateFilter('action', e.target.value);
                debouncedRefresh(100);
              }}
              className="h-9 px-3 pr-8 bg-white border border-slate-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all appearance-none hover:bg-slate-50"
            >
          ...
          <div className="flex items-center space-x-3">
            ...
            {/* Apply Button - Show initial load if no data loaded yet */}
            {!lastRefresh && !filters.lastApplied ? (
              <Button
                type="primary"
                size="medium"
                prefix={<ChartIcon size={16} />}
                onClick={onApplyInitialFilters}
                disabled={loading}
              >
        ...
        {/* Filter Summary Row */}
        {(filterSummary.hasActiveFilters || lastRefresh) && !error && (
            ...
            <div className="flex items-center justify-between">
              ...
              {(lastRefresh || filters.lastApplied) && (
                <div className="flex items-center space-x-2 text-xs text-slate-500">
                  <ClockIcon size={12} />
                  <span>Osveženo: {(lastRefresh || filters.lastApplied).toLocaleTimeString('sr-RS')}</span>
                  {loading && (
                    <>
                      <span>•</span>
